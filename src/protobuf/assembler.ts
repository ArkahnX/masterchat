...
export function csc(videoId: string, { top = false, highlightedCommentId }: CscOptions = {}) {
	...
	return b64e(
		cc([
			...
			vt(3, 6),
			ld(6, [
				ld(4, [ld(4, videoId), vt(6, sortType), vt(15, 2), ...(highlightedCommentId ? [ld(16, highlightedCommentId)] : [])]),
				// vt(5, 50),
...
export function removeMessageParams(chatId: string, origin: CVPair, retract: boolean = true) {
...
export function hideParams(channelId: string, origin: CVPair, undo: boolean = false): string {
	const op = undo ? 5 : 4;
	return b64e(cc([ld(1, cvToken(origin)), ld(op, ld(1, truc(channelId))), vt(10, 2), vt(11, 1)]), B64Type.B2);
}

export function pinParams(chatId: string, origin: CVPair) {
	return b64e(ld(1, [ld(1, cvToken(origin)), ld(2, chatToken(chatId)), vt(3, 1), vt(10, 2), vt(11, 1)]), B64Type.B2);
}

export function unpinParams(actionId: string, origin: CVPair) {
	return b64e(ld(1, [ld(1, cvToken(origin)), ld(2, ld(1, ld(1, actionId))), vt(3, 2), vt(10, 2), vt(11, 1)]), B64Type.B2);
}

export function addModeratorParams(tgt: string, origin: CVPair, undo: boolean = false) {
	// TODO: support undo op
	b64e(cc([ld(1, cvToken(origin)), ld(2, ld(1, truc(tgt)))]), B64Type.B2);
}

export function getContextMenuParams(chatId: string, authorChannelId: string, origin: CVPair): string {
	return b64e(cc([ld(1, chatToken(chatId)), ld(3, cvToken(origin)), vt(4, 2), vt(5, 4), ld(6, ld(1, authorChannelId))]), B64Type.B2);
}

export function getTranscriptParams(videoId: string, language: string, autoGenerated: boolean = true) {
	return b64e(
		cc([
			ld(1, videoId),
...
export function transcriptFormatToken(language: string, autoGenerated: boolean = true) {
	return b64e(cc([ld(1, autoGenerated ? "asr" : ""), ld(2, language), ld(3, "")]), B64Type.B1);
}

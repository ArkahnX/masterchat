...
const credentials = credentialsB64 ? JSON.parse(Buffer.from(credentialsB64, "base64").toString()) : undefined;
const credentials_second = credentialsB64_second ? JSON.parse(Buffer.from(credentialsB64_second, "base64").toString()) : undefined;

const enabled = id && channelId && channelId_second && credentials && credentials_second;
if (!enabled) {
	...
	describe("check message", () => {
		itif("receive message", async () => {
			...
			const chat = res.actions.find(
				(action): action is AddChatItemAction => action.type === "addChatItemAction" && action.id === chatId
			);
...
describe("moderation (timeout)", () => {
	...
	describe("verify timeout", () => {
		itif("try sending chat", async () => {
			await expect(other.sendMessage("Hi!")).rejects.toThrow("You have been placed in timeout");
		});
	});
});

...
export function parseMarkChatItemAsDeletedAction(payload: YTMarkChatItemAsDeletedAction) {
	...
	switch (statusText) {
		...
		case "Message deleted by ":
			break;
		default:
			debugLog("[action required] Unrecognized deletion status:", statusText, JSON.stringify(payload));
	}

	const executor = statusText === "Message deleted by " ? payload.deletedStateMessage.runs[1].text : undefined;

...
export function endpointToUrl(navigationEndpoint: NonNullable<YTTextRun["navigationEndpoint"]>): string | undefined {
	...
	if ("watchEndpoint" in navigationEndpoint) {
		...
		if (watchEndpoint.startTimeSeconds) url += "&t=" + watchEndpoint.startTimeSeconds;
	...
	if ("browseEndpoint" in navigationEndpoint) {
		...
		if ("canonicalBaseUrl" in browseEndpoint) {
			return stripYtRedirection((browseEndpoint as FluffyBrowseEndpoint).canonicalBaseUrl);
		} else if (browseId) {
 ...
 */
export function stringify(payload: YTText | YTRun[] | string, runsToStringOptions?: RunsToStringOptions): string;
export function stringify(payload: undefined, runsToStringOptions?: RunsToStringOptions): undefined;
export function stringify(payload: YTText | YTRun[] | string | undefined, runsToStringOptions?: RunsToStringOptions): string | undefined {
	// undefined
...
export function simpleTextToString(payload: YTSimpleTextContainer, expand: boolean = false) {
...
export function getTimedContinuation(continuationContents: YTContinuationContents): TimedContinuation | undefined {
	/**
...
export type OmitTrackingParams<T> = Omit<T, "clickTrackingParams" | "trackingParams">;

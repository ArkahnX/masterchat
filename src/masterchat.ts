...
export class Masterchat extends EventEmitter {
	...
	private async postWithRetry<T>(input: string, body: any, options?: RetryOptions): Promise<T> {
			...
			try {
				...
				if (err instanceof Error) {
					...
					if (remaining > 0) {
						...
						remaining -= 1;
						debugLog(`Retrying(postJson) remaining=${remaining} after=${retryInterval}`);
						continue;
					}
	...
	private async get<T>(input: string, config: AxiosRequestConfig = {}): Promise<T> {
		if (!input.startsWith("http")) {
			input = Constants.DO + input;
		}
	 ...
	 */
	private async getActionCatalog(contextMenuEndpointParams: string): Promise<ActionCatalog | undefined> {
		const query = new URLSearchParams({
			params: contextMenuEndpointParams,
		});
		const endpoint = Constants.EP_GICM + "&" + query.toString();
		const response = await this.postWithRetry<YTGetItemContextMenuResponse>(endpoint, withContext(), {
			retry: 2,
		});
		...
		for (const item of response.liveChatItemContextMenuSupportedRenderers!.menuRenderer.items) {
			...
			switch (text) {
				...
				case "Block": {
					const endpoint =
						item.menuNavigationItemRenderer!.navigationEndpoint.confirmDialogEndpoint!.content.confirmDialogRenderer
	 ...
	 */
	public static async init(videoIdOrUrl: string, options: MasterchatOptions = {}) {
	 ...
	 */
	constructor(videoId: string, channelId: string, { mode, credentials, axiosInstance }: MasterchatOptions = {}) {
		super();
	...
	public async fetch(tokenOrOptions?: string | FetchChatOptions, maybeOptions?: FetchChatOptions): Promise<ChatResponse> {
	...
	public async sendMessage(message: string): Promise<YTLiveChatTextMessageRenderer> {
		...
		const res = await this.postWithRetry<YTActionResponse>(Constants.EP_SM, body);
	...
	public async getComment(commentId: string) {
		...
		if (first.renderingPriority !== RenderingPriority.LinkedComment) return undefined;
	...
	public async getComments(continuation: string | CscOptions = {}) {
		...
		const comments = items.map((item) => item.commentThreadRenderer).filter((rdr): rdr is YTCommentThreadRenderer => rdr !== undefined);

		return {
			comments,
			continuation: nextContinuation,
			next: nextContinuation ? () => this.getComments(nextContinuation) : undefined,
		};
	}
	...
	public async getTranscript(language: string): Promise<TranscriptSegment[]> {
		const fetchTranscript = async ({ autoGenerated }: { autoGenerated: boolean }) => {
			...
			const rdr = res.actions[0].updateEngagementPanelAction.content.transcriptRenderer.content.transcriptSearchPanelRenderer;

			const subtitles = rdr.footer?.transcriptFooterRenderer.languageMenu?.sortFilterSubMenuRenderer.subMenuItems;
	...
	public async getPlaylist(browseId: string | { type: "membersOnly" }) {
		...
		const res = await this.post<any>("https://www.youtube.com/youtubei/v1/browse", {
			context: {
		...
		const contents =
			res.contents.twoColumnBrowseResultsRenderer.tabs[0].tabRenderer.content.sectionListRenderer.contents[0].itemSectionRenderer
